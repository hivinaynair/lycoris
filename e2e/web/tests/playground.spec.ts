...
test("all simulated outcomes use the SDK without network payments", async ({ page }) => {
  const paymentRequests: string[] = [];
  page.on("request", (request) => {
    if (request.url().includes("sepolia.base.org") || request.url().includes("/api/settle-kit"))
      paymentRequests.push(request.url());
  });
  await page.goto("/checkout");
  for (const scenario of ["success", "rejected", "insufficient", "delayed"]) {
    await page.getByLabel("02 / Payment scenario").selectOption(scenario);
    await page.getByRole("button", { name: "Buy", exact: true }).click();
    await expect(page.getByRole("button", { name: "Pay 0.1 USDC" })).toBeFocused();
    ...
    if (scenario === "delayed") {
      ...
      await expect(page.getByRole("button", { name: "Wallet", exact: true })).toBeDisabled();
      await page.getByRole("button", { name: "Merchant theme", exact: true }).click();
      await page.getByRole("button", { name: "Check payment status" }).click();
    }
    await expect(page.getByTestId("checkout-state")).toHaveText(
      ["success", "delayed"].includes(scenario) ? "settled" : "failed",
    );
    await expect(page.getByTestId("simulation-sends")).toHaveText(
      ["success", "delayed"].includes(scenario) ? "1" : "0",
    );
    await expect(page.getByRole("heading", { name: "Sample report unlocked" })).toHaveCount(
      ["success", "delayed"].includes(scenario) ? 1 : 0,
    );
