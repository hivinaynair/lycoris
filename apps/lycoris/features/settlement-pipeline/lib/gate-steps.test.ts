...
describe("gateStepsForResult", () => {
  it("emits settlement gates on success", () => {
    expect(gateStepsForResult(undefined, "0xabc")).toEqual([2, 3, 4]);
  });
  ...
  it("stops at settlement gate for invalid_exact_evm errors", () => {
    ...
    expect(gateStepsForResult("invalid_exact_evm_some_other_error", undefined)).toEqual([2, 3, 4]);
    expect(gateStepsForResult("settlement_transaction_failed", undefined)).toEqual([2, 3, 4]);
    expect(gateStepsForResult("settlement_receipt_unconfirmed", undefined)).toEqual([2, 3, 4]);
  });
