...
/**
 * The slice of a 4337 stack this adapter needs: who is paying, and something
 * that will submit a user operation for them.
 *
 ...
 * SDK asks for an address and a way to send a transaction; it gets both, and
 * nothing it can see distinguishes this from an EOA. No word of the 4337
 * execution model — user operations, bundlers, the EntryPoint, paymasters — crosses
 * this boundary, which is the claim `PaymentSigner`'s two fields were making.
 *
 ...
 * `confirm()`'s job, and `createUserOpReceiptClient` does it by
