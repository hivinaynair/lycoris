...
export async function payForResourceTool(
  ...
  if (!local.ok) {
    const code = local.reason.startsWith("expired") ? "mandate_expired" : `mandate_${local.reason}`;
    const reason = local.reason === "expired" ? "mandate_expired" : `mandate_${local.reason}`;
    return jsonError(code, { reason, payId });
  }
