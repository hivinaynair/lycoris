...
function requireManager(manager: CheckoutManager | null, verb: string): CheckoutManager {
  if (!manager) {
    throw new Error(`begin() before ${verb}()`);
  }
  return manager;
}
...
export function useCheckout(options?: {
  ...
  if (!ctx) {
    throw new Error("useCheckout must be used inside SettleProvider");
  }
  ...
  return { state, title: ctx.title, begin, selectMethod, pay, reset };
}
