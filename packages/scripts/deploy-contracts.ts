...
async function deploy(name: string): Promise<`0x${string}`> {
  const { abi, bytecode } = JSON.parse(
    readFileSync(resolve(process.cwd(), `contracts/lycoris/artifacts/${name}.json`), "utf8"),
  );
...
console.log("↳ Add the above line to packages/scripts/.env.local and apps/facilitator/.env.local");
