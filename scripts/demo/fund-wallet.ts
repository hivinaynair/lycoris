...
function usage() {
  console.log(
    `
Usage:
  bun --env-file=scripts/.env.local scripts/demo/fund-wallet.ts <evm-address> [--token eth|usdc]

Examples:
  bun --env-file=scripts/.env.local scripts/demo/fund-wallet.ts 0x1234...abcd
  bun --env-file=scripts/.env.local scripts/demo/fund-wallet.ts 0x1234...abcd --token usdc
`.trim(),
  );
}
